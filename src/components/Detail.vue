<template>
  <div class="detail-width">
    <div class="d-flex p4 justify-space-between border-bottom">
      <div class="display-column align-start">
        <span class="text-simple">{{
          itemSelect ? "Usuario seleccionado" : "Crear Usuario"
        }}</span>
        <span class="text-bold">{{ dataDetail ? dataDetail.name : "" }}</span>
      </div>
      <div class="d-flex content-btn">
        <button class="btn primary" @click="addUser()">
          <span>{{itemSelect ? "Actulizar" : 'Guardar'}}</span>
        </button>
        <button class="btn btn-error" @click="deleteUser()">
          <div class="btn-icon">
            <img src="../assets/minus.svg" height="15" alt="img delete"  class="filter-white" />
          </div>
          <span>Eliminar</span>
        </button>
      </div>
    </div>
    <div class="container-detail">
      <div class="p4 content-img">
        <img class="circle-big" :src="dataDetail ? dataDetail.image : image" />
        <span class="p4 text-simple">Beneficiario</span>
      </div>
      <div class="content-forms">
        <div class="display-column align-start p-input">
          <span class="text-simple">Nombre completo</span>
          <input class="input-forms" type="text" v-model="name" />
        </div>
        <div class="display-column align-start p-input">
          <span class="text-simple">No. de tarjeta </span>
          <input class="input-forms" type="text" v-model="document" />
        </div>
        <div class="display-column align-start p-input">
          <span class="text-simple">Saldo</span>
          <input class="input-forms" type="text" v-model="cash" />
        </div>
      </div>
    </div>
    <deleteModal :visible="deleteData.active" :user="dataDetail" />
    <alert :visible="alert.active" :message="alert.msg" />
  </div>
</template>

<script>
import deleteModal from "@/components/DeleteModal.vue";
import alert from "@/components/Alert.vue";

export default {
  name: "detail",
  components: {
    alert,
    deleteModal,
  },
  data: () => ({
    dataDetail: null,
    name: null,
    document: null,
    cash: null,
    alert: {
      msg: null,
      active: false,
    },
    deleteData: {
      active: false,
    },
    image:
      "data:image/png;base64,",
  }),
  methods: {
    addUser() {
      if (this.name && this.document && this.cash) {
        const temp = this.beneficiary.filter((p) => p.id == this.itemSelect);
        if (temp && this.itemSelect) {
          this.alert.active = true;
          this.alert.msg = "El Beneficiario a sido actualizado ";
          setTimeout(() => {
            this.alert.active = false;
          }, 1000);
        } else {
        //Se crea el usuario utilizando la mutations 'PUSH_USER'
          this.$store.commit("PUSH_USER", {
            name: this.name,
            document: this.document,
            cash: this.cash,
            id: this.idIncrement,
            image: this.image,
          });
          this.itemSelect = this.idIncrement;
          this.idIncrement ++
        }
      } else {
        this.alert.active = true;
        this.alert.msg = "Complete los campos para guardar";
        setTimeout(() => {
          this.alert.active = false;
        }, 1000);
      }
    },
    deleteUser() {
    //Se habilita el modal de confirmacion para eliminar
      if (this.itemSelect) {
        this.deleteData.active = true;
        setTimeout(() => {
          this.deleteData.active = false;
        }, 1000);
      } else {
        this.alert.active = true;
        this.alert.msg = "Seleccione el usuario que desea eliminar";
        setTimeout(() => {
          this.alert.active = false;
        }, 1000);
      }
    },
    clearData() {
      this.name = null;
      this.document = null;
      this.cash = null;
    },
  },
  computed: {
    beneficiary: {
      get() {
        return this.$store.state.beneficiary || [];
      },
    },
    idIncrement:{
      get() {
        return this.$store.state.idIncrement || null;
      },
      set(value) {
        this.$store.commit("SET_EDITING_OBJECT", {
          key: "idIncrement",
          value,
        });
      },    
    },
    itemSelect: {
      get() {
        return this.$store.state.itemSelect || null;
      },
      set(value) {
        this.$store.commit("SET_EDITING_OBJECT", {
          key: "itemSelect",
          value,
        });
      },
    },
  },
  watch: {
    itemSelect() {
      if (this.itemSelect) {
        const temp = this.beneficiary.filter((p) => p.id == this.itemSelect);
        temp ? (this.dataDetail = temp[0]) : (this.dataDetail = null);
        this.name = this.dataDetail.name;
        this.document = this.dataDetail.document;
        this.cash = this.dataDetail.cash;
      } else {
        this.dataDetail = null;
        this.clearData();
      }
    },
  },
};
</script>
<style>
.container-detail {
  display: flex;
  flex-direction: row;
  align-items: start;
  padding-top:4%;
}

.content-img {
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  width: 20%;
}

.content-forms {
  border-left: solid #e5e5e5 1px;
  width: 80%;
}
</style>
